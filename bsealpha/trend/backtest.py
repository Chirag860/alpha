...
def backtest(weights: np.ndarray, ret: np.ndarray, params: TrendParams, *,
    ...
    """Run the daily backtest. ``weights`` are targets at each day; ``ret`` are simple returns."""
    weights = np.asarray(weights, dtype=float)
    ret = np.asarray(ret, dtype=float)
    T, N = weights.shape
    daily_target = float(params.target_ann_vol) / np.sqrt(252.0)

    # -- realized-vol overlay: tighten to the target using the strategy's own trailing vol -----
    if params.vol_target_overlay:
        base = np.zeros(T)
        base[1:] = np.sum(weights[:-1] * ret[1:], axis=1)
        trail = _ewma_std(base, params.vol_halflife)
        scale = np.clip(daily_target / np.maximum(trail, 1e-9), 0.2, 3.0)
        scale[: min(20, T)] = 1.0                      # warmup: don't scale on noise
        w = weights * scale[:, None]
    else:
        w = weights.copy()

    # re-apply caps after scaling
